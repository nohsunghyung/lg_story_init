...
function serve(done) {
  server.init({
    port: 9000,
    files: ["./*.{html}", "local/**/*.{css,js,img}"],
    server: { baseDir: "./" },
    startPath: "local/index.html",
    browser: "chrome",
    reloadDelay: 800,
  });
  done();
...
var watch = () => {
  ...
  gulp.watch(paths.html.watch, gulp.series(reload));
  gulp.watch(paths.js.src);
};

var dev = gulp.series(css_compile_dev, serve, watch);
var build = gulp.series(css_clean, css_compile_build);
